...
//            let downloadURL = metadata.downloadURL()
    ...
    @IBAction func onSubmitPressed(_ sender: UIButton) {
        ...
        let submitAction = UIAlertAction(title: "Submit",
                                       style: .default) { _ in
            // 1
            guard let text = self.locationInput.text else {return}
            ...
            self.rootRef.child("reports/" + text).observeSingleEvent(of: .value, with: { (snapshot) in
                //update votes if street report exists
                if snapshot.exists(){
                    self.updateCount(str: text)
                }else{ // make new report
                    // 2
                    let report = Report(type: self.reportType, street: text, ward: wardText, votes: 1)
